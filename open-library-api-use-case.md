...
2. Execute the [Search API](the-open-library-search-api) to find data about those books with one call! You do this by logically OR'ing the ISBN values in the [query value](#query-parameter-qquery). Be sure to set the [fields value](#query-parameter-fieldsfields) to select the fields needed, as [discussed earlier](#use-case-fields). The Search API's request URL query parameters should be like :warning: *(this is a long line, scroll right as needed)*:
